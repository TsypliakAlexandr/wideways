...
function septera_featured_width() {
   ...
   if ( is_front_page() && $options['septera_landingpage'] ) {
       ...
       $width = ceil( (int)$options['septera_sitewidth'] / (int)$options['septera_magazinelayout'] );
   }
...
function septera_back_top() {
	echo '<a id="toTop"> <span class="screen-reader-text">' . __('Back to Top', 'septera') . '</span> <i class="icon-back2top"></i> </div>';
} // septera_back_top()
...
function septera_get_sidebar() {
	global $post;
	if ( get_post() && is_singular() ) { $septera_meta_layout = get_post_meta( $post->ID, '_septera_layout', true ); }
